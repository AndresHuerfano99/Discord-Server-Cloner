...
def print_add(message: str) -> None:
    """Prints a message indicating an addition/creation event."""
    print(f"{Style.BRIGHT}{Fore.CYAN}{message}{Fore.RESET}")


def print_delete(message: str) -> None:
    """Prints a message indicating a deletion event."""
    print(f"{Style.BRIGHT}{Fore.CYAN}{message}{Fore.RESET}")
...
class Clone:
    ...
    async def channels_create(guild_to: discord.Guild, guild_from: discord.Guild) -> None:
        ...
        for channel_text in guild_from.text_channels:
            try:
                category = next(
                    (cat for cat in guild_to.categories if cat.name == channel_text.category.name), None
                )
                overwrites_to = {
                    discord.utils.get(guild_to.roles, name=role.name): overwrite
        ...
        for channel_voice in guild_from.voice_channels:
            try:
                category = next(
                    (cat for cat in guild_to.categories if cat.name == channel_voice.category.name), None
                )
                overwrites_to = {
                    discord.utils.get(guild_to.roles, name=role.name): overwrite
